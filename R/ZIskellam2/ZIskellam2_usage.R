...
ZIskellam2_path <- "C:/Users/lntre/Desktop/results/ZIskellam2"
...
for (j in seq_along(leagues)){
  ...
  for (i in seq_along(seasons)){
    ...
    ZIskellam2_model <- fit_ZIskellam2(z_df = z_df)
    fitting_time = Sys.time() - t0

    message("Fitting time:", fitting_time)

    # Check the results:
    ZIskellam2_results <- summarize_ZIskellam2(model = ZIskellam2_model, z_df = z_df)

    ZIskellam2_fitted_league <- league_probabilities_ZIskellam2(z_df, ZIskellam2_model)
    ...
    message("Simulation:")
    t0 = Sys.time()
    sim_array <- simulate_season_ZIskellam2(
      B = B, z_df = z_df, model = ZIskellam2_model
    )
    ...
    folder_path <- file.path(ZIskellam2_path, paste0(leagues[j], seasons[i]))
