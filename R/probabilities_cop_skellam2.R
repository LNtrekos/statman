fit_match_cop_skellam2 <- function(
    Z_range, mu1, mu2,
    sigma1, sigma2, copula, theta){
...
fit_league_cop_skellam2 <- function(z_df, model){
  # Model matrix
  ...
  # sigma^2:
  sigma1_hat <- tail(model$model$par, 3)[1]
  sigma2_hat <- tail(model$model$par, 3)[2]
  theta_hat <- tail(model$model$par, 1)
  ...
  fitted_probs <- t(sapply(
    1:nrow(mm),
    function(i) fit_match_cop_skellam2(
      Z_range, mus[i], mus[i], sigma1_hat, sigma2_hat, frank_pcop, theta_hat
    )
  ))
